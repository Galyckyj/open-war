...
export function useGameSocket(
  ...
  const tryJoinGame = () => {
    ...
    const payload = JSON.stringify({ type: 'input', playerId, payload: { type: 'join', name: nickname.trim() } });
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      statsRef.current.bytesIn += typeof raw === 'string' ? byteLength(raw) : (raw as ArrayBuffer).byteLength;
      try {
        ...
        if (msg.type === 'state' && msg.payload) {
          ...
          setUiSnapshot({ players: msg.payload.players, tick: msg.payload.tick });
          return;
        }
        if (msg.type === 'tick' && typeof msg.tick === 'number') {
          ...
          for (const [i, ownerId] of delta) {
            if (curr.cells[i]) curr.cells[i] = { ...curr.cells[i]!, ownerId: ownerId ?? null };
          }
  ...
  const sendSpawn = (tile: number) => {
    ...
    const payload = JSON.stringify({ type: 'input', playerId, payload: { type: 'spawn', tile } });
    statsRef.current.bytesOut += byteLength(payload);
  ...
  const sendAttack = (targetId: string | null, troops?: number) => {
    if (!playerId || wsRef.current?.readyState !== WebSocket.OPEN) return;
    const payload = JSON.stringify({ type: 'input', playerId, payload: { type: 'attack', targetId, troops } });
    statsRef.current.bytesOut += byteLength(payload);
    wsRef.current.send(payload);
  };
